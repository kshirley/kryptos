...
k_seq <- match(c("K", "R", "Y", "P", "T", "O", "S"), LETTERS)
kk <- (cumsum(rep(k_seq, 14)) - 1) %% 435 + 1
key <- cipher_text[kk][1:97]

decode_vigenere(cipher = K4, key = key, vigenere_table = V_kryptos)
decode_vigenere(cipher = K4, key = key, vigenere_table = V_plain)
